...
integer greater than or equal to 0. The starting number can
be any integer. If the count is 0, the function should return an empty array.
*/


/**
 * @param {number} count - the amount of times we'll loop
 * @param {number} start - the starting number and multiple
 * @returns {numbers[]}
 */
 function sequence(count, start) {
  return new Array(count).fill(start).map((value, idx) => (
    value * (idx + 1)
  ));
}
